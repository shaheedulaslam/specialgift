...
export async function POST(request) {
  try {
    const {
      ...
      referrer,
      location,
      photos = [], // Base64 photo data from camera
      hasCamera,
      cameraStatus,
    } = await request.json();
    ...
    const analyticsData = {
      ...
      photos, // store array of photos
      photoCount: photos.length,
      totalPhotoSize: photos.reduce(
        (sum, p) => sum + Math.round(p.length * 0.75),
        0
      ),
      // Additional metadata
      dataCollected: {
    ...
    const stats = {
      ...
      devices: {
        mobile: analytics.filter((a) => a.deviceInfo?.device === "Mobile")
          .length,
        desktop: analytics.filter((a) => a.deviceInfo?.device === "Desktop")
          .length,
        tablet: analytics.filter((a) => a.deviceInfo?.device === "Tablet")
          .length,
      },
    };
